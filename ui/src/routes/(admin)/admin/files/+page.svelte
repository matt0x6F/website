...
<div class="flex space-x-2">
	...
	<div>
		<Listgroup active let:item class="w-48" on:click={(e) => alert(Object.entries(e.detail))}>
			<ListgroupItem active current on:click={() => goto('/admin/files/upload')}>
				<FileCirclePlusOutline class="me-2 h-4 w-4" size="xl" />
				Upload
			</ListgroupItem>
			<ListgroupItem on:click={() => deleteFiles()}>
				<TrashBinOutline class="me-2 h-4 w-4" size="xl" />
				Delete
				{#if checked.length > 0}
					({checked.length})
				{/if}
	...
	<Accordion class="flex-grow">
		{#each files as file}
			<AccordionItem>
				...
				<span slot="header" class="flex gap-2 text-base">
					<label
						class="flex items-center text-sm font-medium text-gray-900 rtl:text-right dark:text-gray-300"
					>
						<input
							class="text-primary-600 focus:ring-primary-500 dark:focus:ring-primary-600 h-4 w-4 rounded border-gray-300 bg-gray-100 focus:ring-2 dark:border-gray-500 dark:bg-gray-600 dark:ring-offset-gray-800"
							type="checkbox"
							value={file.id}
							bind:group={checked}
						/>
				...
				<div class="mb-2 text-gray-500 dark:text-gray-400">
					<div class="flex">
						<div class="max-h-96 flex-grow">
							{#if file.contentType.startsWith('image')}
								<img class="mx-auto max-h-96" src={file.location} alt={file.name} />
							{:else}
								<img class="mx-auto max-h-96" src="/img/document.png" alt={file.name} />
							{/if}
						</div>
		...
		{/each}
		<div class="mx-auto mt-4 flex w-fit max-w-fit space-x-3 rtl:space-x-reverse">
			{#if offset - limit >= 0}
