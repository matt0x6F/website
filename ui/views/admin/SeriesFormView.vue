      ...
      <div class="mt-10">
        ...
        <ul class="space-y-2">
          <li v-for="post in loadedSeries.posts" :key="post.id" class="flex items-center gap-2">
            <NuxtLink :to="{ name: 'admin-posts-edit-id', params: { id: post.id } }" class="text-blue-700 hover:underline dark:text-blue-300">
              {{ post.title }}
            </NuxtLink>
...
const handleSave = async () => {
  ...
  try {
    if (isEditing.value && seriesId.value) {
      ...
      router.replace({ name: 'admin-series-id', params: { id: newSeries.id.toString() }})
    }
  } catch (error) {
