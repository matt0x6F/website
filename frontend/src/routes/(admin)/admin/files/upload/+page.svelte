...
>
	<Label for="postSelect" class="mb-2 text-sm text-gray-500 dark:text-gray-400"
		>Select post(s)</Label
	>
	<MultiSelect
		id="postSelect"
		items={posts}
		bind:value={selected}
		placeholder="Select a post or posts"
	/>

	<Label for="visibility" class="mb-2 text-sm text-gray-500 dark:text-gray-400">Visibility</Label>
	<Checkbox
		id="visibility"
		checked={visbility === 'public'}
		on:change={() => {
			visbility === 'public' ? (visbility = 'private') : (visbility = 'public');
		}}>Public</Checkbox
	>
